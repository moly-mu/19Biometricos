...
const FaceRecognitionExit = () => {
  ...
  const sendFrameToBackend = async (base64Image) => {
    ...
    try {
      ...
      const response = await axios.post("http://127.0.0.1:8000/facial/salida/", {
        image_base64: base64Image, // Imagen en Base64
      });

      setMessage(response.data.message);

      // Detener la cámara si el backend confirma el reconocimiento
  ...
  return (
    <div className = "container-face-c">
      ...
      <p className="message-face-c">{message}</p> {/* Mensaje del backend */}
    </div>
  );
};

export default FaceRecognitionExit;
